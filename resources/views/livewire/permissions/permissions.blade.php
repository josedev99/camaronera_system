...
<div class="col-lg-12 col-12 col-sm-12 layout-spacing">
                            <div class="statbox widget box box-shadow">
                                ...
                                <div class="widget-content widget-content-area">
                                    @cannot(['permission_edit', 'permission_destroy', 'permission_search', 'permission_create'])
                                        ...
                                        <table class="table table-bordered table-hover mb-4">
                                            <thead style=" background: #f37f23">
                                                <tr>
                                                    ...
                                                    @if(auth()->user()->can('permission_edit') || auth()->user()->can('permission_destroy'))
                                            ...
                                            @foreach($permissions as $permision)
                                                <tr>
                                                    <td class="text-center">{{$permision->id}}</td>
                                                    ...
                                                    <td class="text-center">
                                                        @can('permission_edit')
                                                        <a href="javascript:void(0)" wire:click="Edit({{$permision->id}})" style="border: none !important" class="btn btn-outline-info btn-sm" title="Editar"><i class="fas fa-edit mt-1"></i></a>
                                                        @endcan

                                                        @can('permission_destroy')
                                                        <a href="javascript:void(0)" onclick="Confirm('{{$permision->id}}')" style="border: none !important" class="btn btn-outline-danger btn-sm" title="Eliminar"><i class="fas fa-trash mt-1"></i></a>
                                                        @endcan
...
<script>
    document.addEventListener('DOMContentLoaded', function(){
        ...
        window.livewire.on('permission-add', msg => {
            $('#theModal').modal('hide');
            noty(msg);
        });
        ...
        window.livewire.on('permission-delete', msg => {
             noty(msg);
        });
